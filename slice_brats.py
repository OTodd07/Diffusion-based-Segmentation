...
valid_ids = ['HGG/Brats18_CBICA_AQY_1/Brats18_CBICA_AQY_1',
...
def slice_data(input_dir, output_dir):
    splits = [(train_ids, 'training'), (test_ids, 'testing')]
    ...
    for split in splits:
        ...
        for (idx, split_id) in enumerate(split_ids):
            grade, pat, _ = split_id.split('/')
            curr_dir = os.path.join(input_dir, grade, pat)
            print(curr_dir)
            volumes = {}
            for f in os.listdir(curr_dir):
                ...
                if mode == "mask":
                    continue
                arr = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(curr_dir, f)))
                volumes[mode] = arr
            for i in range(49):
                slice_dir = os.path.join(output_dir, split[1], f"{slice_id:05d}")
                os.makedirs(slice_dir)
                for k in volumes:
                    sitk.WriteImage(sitk.GetImageFromArray(volumes[k][80+i]), os.path.join(slice_dir, f"brats_{split[1]}_{(idx+1):03d}_{k}_{(80+i):03d}_w.nii.gz"))
                slice_id += 1



if __name__=="__main__":
    input_dir = "brats18-preprocessed"
    output_dir = "brats18_processed_slices"
    slice_data(input_dir, output_dir)
